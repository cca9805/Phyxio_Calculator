...
export const calculators = [
  {
    ...
    variables: [
      ...
      { symbol: 'theta', label: 'Ángulo entre Fuerza y Brazo', unit: '°', example: '90' },
    ],
    output: { symbol: 'M', label: 'Momento (Torque)', unit: 'N·m' },
    resolve: (args) => {
      const F = Number(args.F), r = Number(args.r), theta = Number(args.theta);
      if ([F, r, theta].some(isNaN)) return { error: 'Valores inválidos.' };
      const M = F * r * Math.sin(degToRad(theta));
      return { result: { M }, steps: [
        `$M = F \\cdot r \\cdot \\sin(\\theta) = ${formatNumber(F)} \\cdot ${formatNumber(r)} \\cdot \\sin(${theta}°) = ${formatNumber(M)} N\\cdot m$`
      ] };
    }
  },
  {
    ...
    formula: 'M_O = (Fy·x) - (Fx·y)',
    variables: [
      { symbol: 'Fx', label: 'Componente Fx de la Fuerza', unit: 'N', example: '30' },
      { symbol: 'Fy', label: 'Componente Fy de la Fuerza', unit: 'N', example: '40' },
    ...
    resolve: (args) => {
      const Fx = Number(args.Fx), Fy = Number(args.Fy), x = Number(args.x), y = Number(args.y);
      if ([Fx, Fy, x, y].some(isNaN)) return { error: 'Valores inválidos.' };
      const M_O = (Fy * x) - (Fx * y);
      return { result: { M_O }, steps: [
        `$M_O = (F_y \\cdot x) - (F_x \\cdot y) = (${formatNumber(Fy)} \\cdot ${formatNumber(x)}) - (${formatNumber(Fx)} \\cdot ${formatNumber(y)})$`,
        `$M_O = ${formatNumber(Fy * x)} - ${formatNumber(Fx * y)} = ${formatNumber(M_O)} N\\cdot m$`
      ] };
    }
  },
  {
