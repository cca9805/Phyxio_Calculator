...
import {
  ...
  ejerciciosMCU,
  simuladorMCU
} from '../../../../data/cinematica/MCU';

// Importar componentes
import examplesFromFile from './MCUSections/Examples';
import TheoryComponent from './MCUSections/Theory';

// Obtener la configuración de MCU
const mcuConfig = calculatorConfigs.mcu;

function Calculator() {
  const [values, setValues] = useState({});
  const [result, setResult] = useState(null);
  const [selectedMode, setSelectedMode] = useState(mcuConfig.modes[0].id);
  
  const currentMode = mcuConfig.modes.find(mode => mode.id === selectedMode);
...
export default function MCU() {
  const subtopicProps = createSubtopicProps({
    ...
    intro: <p>Movimiento en trayectoria circular con rapidez constante.</p>,
    headerVariant: "mp",
    sectionVariant: "card",
